...
namespace aliyun_net_credentials_unit_tests
{
    public class RsaKeyPairCredentialTest
    {
        ...
        public async Task RsaKeyPairTest()
        {
            ...
            mockProvider.Setup(p => p.GetCredentials()).Returns(new RsaKeyPairCredential("publicKeyId", "privateKeySecret", 64090527132000L, null));
            mockProvider.Setup(p => p.GetCredentialsAsync()).ReturnsAsync(new RsaKeyPairCredential("publicKeyId", "privateKeySecret", 64090527132000L, null));
